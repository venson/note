...
# 0.276sec  without index on where
...
# 0.00046sec with index on where 
...
# 0.00046sec with index on group by 
...
# 0.672 sec without index on group by
...
LIMIT 100;  #5.212s
# 4.001 sec with single index on group by and order by
#修改sql_mode
...
ORDER BY create_time DESC 
LIMIT 100;  #0.257s
# 1.530 sec with combianed index on group by and order by
#再进一步：
ALTER TABLE student_info
...
ORDER BY create_time DESC 
LIMIT 100;  #3.790s
# 2.817sec with index on group by and order by (desc as order by)
...
# 0.776sec update with where clause (without index on where)
...
# 0.00044sec update with where clause (with index on where)
...
WHERE NAME = '462eed7ac6e791292a79'; #0.001s
# 0.00051sec join with where clause (index on where ) 
...
# 0.258sec join with where clause(without index on where)
